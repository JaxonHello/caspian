...
dir="TAD_results/GM2878/50kb"
res = 50000
hicname="chr1_kr_50kb"
chr="1"

hicfile=dir+"/"+hicname+".npy"
cellline="GM12878"
...
def getctcf(factorname,chr):
    filename = "chip-seq/GM12878/{}.bed".format(factorname)
    ctcf=[]
...
for metricName in metricList:
    ...
    np.savetxt(dir+"/{}_{}_{}_{}.label".format(cellline,hicname, res,metricName), label)
    boundaries=boundaryPlot(label)
    print("boundaries:",boundaries)
    np.savetxt(dir+"/{}_{}_{}_{}.boudary".format(cellline,hicname, res,metricName), boundaries)
    tadfile=dir+"/{}_{}_{}_{}.tad".format(cellline,hicname, res,metricName)
    i = 0
    ...
    for factor in factors:
        ...
        with open(qualityFile,'a+') as f:
            f.write("\t".join(("chr",str(chr),str(metricName),str(factor),str(runtime),str(lentad),str(achorCount),str(countratio)))+'\n')
            f.close()
